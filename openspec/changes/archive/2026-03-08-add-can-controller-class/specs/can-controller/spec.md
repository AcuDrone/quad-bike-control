# Capability: CAN Controller

## ADDED Requirements

### Requirement: CAN Controller OBD-II Data Reading
The system SHALL interface with vehicle CAN bus using MCP2515 SPI controller to read standard OBD-II diagnostic data.
...
**When** `update()` is called and 100ms has elapsed since last RPM request
...
**Given** the CAN controller is initialized
**When** `update()` is called and 100ms has elapsed since last speed request
...
**Given** the CAN controller is initialized
**When** `update()` is called and 1000ms has elapsed since last temperature request
...
The system SHALL detect and recover from CAN communication errors without crashing or blocking the main control loop.

#### Scenario: Handle OBD-II response timeout
**Given** an OBD-II request has been sent
**When** 1000ms elapses without a response
**Then** the request shall timeout
**And** `VehicleData.dataValid` shall be set to `false`
