...
- [x] 1.1 In the MAVLink timing block beside `MAVLINK_REPORT_TX_MS` (line 215), add
...
- [x] 1.2 Add `MAVLINK_ESC_INDEX` (`0`) and `MAVLINK_ESC_COUNT` (`1`) in the same block, commented
...
- [x] 1.3 **No new runtime-settable value** — no NVS key, no web command, no parameter. Grep the
  diff to confirm `Constants.h` gained only compile-time defines.

## 2. Driver counters (`include/IMotorDriver.h`, `VescMotorDriver`)
- [x] 2.1 `include/IMotorDriver.h`: add two optional telemetry hooks beside the existing ones
...
- [x] 2.2 `include/VescMotorDriver.h`: private `uint16_t replyCount_;` and
  `uint32_t faultEventCount_;` in the telemetry-state block beside `haveReply_`; override both
  accessors inline.
- [x] 2.3 Initialise both to 0 in the constructor initialiser list (`src/VescMotorDriver.cpp`).
- [x] 2.4 `VescMotorDriver::handlePacket()` (`src/VescMotorDriver.cpp:133-140`): inside the
...
- [x] 2.5 Confirm by inspection that both counters are written **only** in `handlePacket()` — never
  in `begin()`, `poll()`, `stop()` or `setSpeed()` — so they are boot-cumulative and a stalled
  `replyCount_` is itself the "VESC silent" diagnostic.
- [x] 2.6 `include/SteeringController.h`: forward both beside the existing VESC telemetry
...
- [x] 3.1 `include/MavlinkInterface.h`: extend `StateReport` (line 71) with a commented
...
- [x] 3.2 `include/MavlinkInterface.h`: in the same block add the steering **position** fields,
...
- [x] 3.3 `include/MavlinkInterface.h`: private `uint32_t lastEscInfoTx_;` beside
  `lastHeartbeatTx_`, and a private helper
  `uint16_t mapVescFaultToEscFlags(uint8_t faultCode) const;`.
- [x] 3.4 Initialise `lastEscInfoTx_(0)` in the constructor list (`src/MavlinkInterface.cpp:29`
  neighbourhood) and clear it in `begin()` alongside the other transmit timestamps.
- [x] 3.5 `src/main.cpp`: fill the new `StateReport` fields from
...
- [x] 3.6 `src/main.cpp`: fill the three steering-position fields from the same
...
- [x] 3.7 `src/MavlinkInterface.cpp`, inside the existing
...
- [x] 3.8 Write the comment block above the pack, in the style of the `EFI_STATUS` block
...
- [x] 3.9 In the same comment block, give `rpm[0]` its own paragraph — it is the one repurposed
...
- [x] 3.10 `src/MavlinkInterface.cpp`: add the `ESC_INFO` block on its own timer, following the
...
- [x] 3.12 Implement `mapVescFaultToEscFlags()` as a `switch` over the VESC `mc_fault_code`:
...
- [x] 3.13 Confirm both messages are sent **unconditionally of link state and of
...
- [x] 3.14 Confirm nothing in the steering control path reads the new fields: grep
...
- [x] 4.1 Add a `### Steering ESC telemetry (ESC_STATUS / ESC_INFO)` subsection under
  "Vehicle state reported back to the autopilot", after the odometer/trip subsection (line 159
  neighbourhood).
- [x] 4.2 Add rows to the main reporting table (line 64 neighbourhood): `Steering ESC input
...
- [x] 4.3 Document field by field: `index = 0` is the **steering** ESC (not a propulsion motor);
...
- [x] 4.4 Give `rpm[0]` its own paragraph: it is the **MEASURED steering position in
  ...
  "RPM".
- [x] 4.5 Document the validity conventions in one place: `NaN` in `voltage[0]`/`current[0]` and
  ...
  meaningful.
- [x] 4.6 In the same place, document `rpm[0] = INT32_MIN` (`-2147483648`) as "steering position
...
- [x] 4.7 Include the VESC-fault → `ESC_FAILURE_FLAGS` mapping table from design.md, with the
  under-voltage caveat and the firmware-version caveat stated in the document itself.
- [x] 4.8 In `### Ground-station plugin compatibility` (line 219), add a short **non-breaking**
  ...
  **not** populate its native `esc1_*` fields from component 25 — the raw packet is delivered by
  ...
  `EFI_STATUS`.
- [x] 4.9 `WEB_PORTAL_SETUP.md` is **not** changed — the portal already shows steering current,
  FET temperature, fault code and driver-ok, and this change adds no web field.

## 5. Build and static checks
- [x] 5.1 `pio run -e esp32-s3-devkitc-1` completes with no errors and no new warnings from `src/`
  or `include/`.
- [x] 5.2 Confirm `ESC_STATUS`/`ESC_INFO` compile from the already-included dialect
  (`<ardupilotmega/mavlink.h>` → `common/`) with **no** new include, no new library and no
  `platformio.ini` change.
- [x] 5.3 `grep -n 'micros()' src/MavlinkInterface.cpp` returns nothing on either new path — both
  timestamps come from `esp_timer_get_time()`.
- [x] 5.4 Confirm the two new `StateReport` fields blocks are filled on **every** call site of the
