...
The system SHALL read vehicle speed from a 3-pin hall-effect speed sensor (VCC, GND, signal) whose
signal reaches the ESP32-S3 **directly** on `PIN_SPEED_SENSOR` (GPIO 17). On this hand-wired
DevKitC-1 there is no opto-isolated input stage, so the firmware SHALL make no assumption about
signal inversion and SHALL treat the pin as a plain digital pulse input.

#### Scenario: Signal reaches the GPIO directly
- **WHEN** the speed sensor signal is connected
- **THEN** it SHALL be wired to `PIN_SPEED_SENSOR` (GPIO 17), the pin freed when the BTS7960
  steering driver was removed
- **AND** GPIO 18 SHALL remain free/reserved and SHALL NOT be used for the sensor
- **AND** the exact wiring (level shifting / pull-up) is **to be confirmed on the bench** and is not
  fixed by this specification

#### Scenario: A 12V sensor must be level-shifted
- **WHEN** a 12V-output sensor is used
- **THEN** external level shifting to 3.3V logic SHALL be fitted before the board is first powered
  with the sensor connected, because GPIO 17 is not 12V tolerant
- **AND** the firmware SHALL NOT attempt to compensate for a missing level shifter
...
- **THEN** a PCNT unit SHALL be configured on `PIN_SPEED_SENSOR` to count a single edge (falling or
  rising — either yields the same pulse rate)
