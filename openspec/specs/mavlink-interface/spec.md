...
states and front light control, driven by commands decoded from the MAVLink interface or the
web portal.
...
- **AND** all relays are initialized to LOW (OFF)
- **AND** internal state tracking is initialized
- **AND** true is returned if initialization succeeds

#### Scenario: Set ignition state
- **WHEN** `setIgnitionState(OFF)` is called, RELAY1 and RELAY2 are set LOW
- **WHEN** `setIgnitionState(ACC)` is called, RELAY1 is LOW and RELAY2 is HIGH
- **WHEN** `setIgnitionState(IGNITION)` is called, RELAY1 and RELAY2 are HIGH
- **AND** internal state is updated to match in each case

#### Scenario: Control front light
- **WHEN** `setFrontLight(true)` is called, RELAY3 is set HIGH and state is updated
- **WHEN** `setFrontLight(false)` is called, RELAY3 is set LOW and state is updated

#### Scenario: Fail-safe all relays off
- **WHEN** `allOff()` is called (fail-safe trigger)
- **THEN** all relays are set LOW
