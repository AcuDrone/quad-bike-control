...
- **Current Use:** CAN_MISO — safe during runtime, only affects boot sequence
- **Warning:** Do not pull LOW during power-up/reset

### NeoPixel Pin (GPIO 8)
- Onboard RGB LED (WS2812)
- **Current Use:** CAN_MOSI — LED may flash during SPI communication (cosmetic only)

### Strapping Pin (GPIO 15)
- Controls ROM boot message verbosity
- **Current Use:** Freed (was GEAR_LOW, now on MCP23017)
- If reused, requires proper initialization with ESP-IDF GPIO API
...
|---------|------|----------|-----------|------------|
| **0** | -- | (freed) | -- | -- |
| **1** | 1 | Throttle Servo | 50 Hz | 16-bit |
| **2** | 4 | Transmission RPWM | 10 kHz | 8-bit |
| **3** | 5 | Transmission LPWM | 10 kHz | 8-bit |
| **4** | 6 | Brake RPWM | 10 kHz | 8-bit |
| **5** | 7 | Brake LPWM | 10 kHz | 8-bit |

---

## UART Allocation

| UART | RX Pin | TX Pin | Baud Rate | Purpose |
|------|--------|--------|-----------|---------|
| **UART0** | 17* | 16* | 115200 | USB Serial Console (reserved) |
| **UART1** | 20 | N/A | 100000 | S-bus Receiver (inverted) |
...
- **Port A (GPA0-7):** Outputs (relays)
- **Port B (GPB0-7):** Inputs with pull-ups (gear sensors, brake sensor)
- **Logic:** Active-low (LOW = sensor active)
- **Code Reference:** `MCP23017Controller`
...
- Baud Rate: 100000 bps, 25 bytes per frame
