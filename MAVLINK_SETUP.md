...
Non-inverted, no inverter circuit (unlike SBUS). Baud is **115200**.

> ⚠ GPIO 17 is **not** free — it is `PIN_SPEED_SENSOR` (hall wheel-speed input).
...
| 4 | ⚑ `fuel_flow` | [cm³/min] Fuel flow | **PHYSICAL (switch-sensed) gear** — same absent-PID justification | **NaN when the gear reads UNKNOWN** — sensor-driven, *not* CAN-driven | `physGearVal` |
...
- **`fuel_flow` = physical gear = measurement** — read from the transmission's gear switches
...
- **`VESC_V` is not `EFI_STATUS.ignition_voltage`.** That field is the **ECU control-module supply**
  on the **12 V** side (PID `0x42`); this one is the **VESC supply rail** that powers the steering
  VESC, measured **by the VESC itself, at the load**. Different rails, different quantities.
...
- **`fuel_flow` alone NaN** → the gear switches are unsure (mid-shift, ambiguous pattern, no
  switch active). Says **nothing** about CAN — "everything is NaN so CAN is down" does not hold in
