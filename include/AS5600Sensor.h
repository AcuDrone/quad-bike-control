...
public:
    ...
    /**
     * @brief Verify the sensor responds on the (already opened) `Wire` bus.
     *
     * The bus is opened once by `main.cpp` and the AS5600 is its only device, so
     * this driver never calls `Wire.begin()` itself. The pins are remembered only
     * for the wedged-bus recovery path.
     *
