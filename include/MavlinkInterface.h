 ...
 * MOTOR current, its input voltage (the VESC supply rail, distinct from
 ...
 * fuel_flow = PHYSICAL (switch-sensed) gear, NaN while unknown — pt_compensation
...
public:
    ...
    struct StateReport {
        ...
        const char* gearPhysical;     // PHYSICALLY sensed gear "R"/"N"/"L"/"H" (gear switches),
                                      // "?" = UNKNOWN (mid-shift / ambiguous / no switch active)
        ...
        float       steerInputVoltageV;   // VESC-measured input voltage, V — the VESC SUPPLY RAIL at
