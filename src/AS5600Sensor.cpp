...
bool AS5600Sensor::begin(gpio_num_t sdaPin, gpio_num_t sclPin) {
    ...
    // it. The AS5600 is the only device on `Wire`. Wire.begin() is idempotent
    // enough to be harmless, but calling it here would let a steering init
    // failure disturb a bus main.cpp already owns.
...
void AS5600Sensor::recoverBus() {
    ...
    // main.cpp owns the bus, and any device added to it later must not find it
    // running at different parameters.
    Debug::printlnFeature(DebugFeature::SERVO, "[AS5600] I2C wedged — re-initializing shared bus");
    Wire.end();
